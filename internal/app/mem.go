...
// MemClient is an in-memory Beads stand-in. It models the parts of bd's
// behaviour LazyBeads depends on — the ready set, blocking semantics and
// dependency direction — so tests can run without a database or a subprocess.
...
// NewMemClient returns an empty in-memory Beads workspace.
func NewMemClient() *MemClient {
	return &MemClient{issues: map[string]*domain.Issue{}, failShow: map[string]error{}}
}

// Add inserts or replaces an issue.
func (f *MemClient) Add(issue domain.Issue) *MemClient {
	if issue.Status == "" {
...
func (f *MemClient) Dep(blocked, blocker string) *MemClient {
	return f.Rel(blocked, blocker, domain.RelBlocks)
}

// Rel records a typed dependency edge.
...
func (f *MemClient) blockersOf(id string) []domain.Dependency {
...
func (f *MemClient) dependentsOf(id string) []domain.Dependency {
	var out []domain.Dependency
	for _, e := range f.edges {
		if e.blocker != id {
			continue
...
func (f *MemClient) isReady(issue *domain.Issue) bool {
...
func (f *MemClient) sortedIssues() []*domain.Issue {
...
func (f *MemClient) Info(context.Context, beads.Scope) (beads.Info, error) {
	return beads.Info{BeadsDir: "/fake/.beads", Prefix: "lb", Mode: "direct"}, nil
}

func (f *MemClient) Ready(_ context.Context, q beads.ReadyQuery) ([]domain.Issue, error) {
	f.record("ready")
...
func (f *MemClient) List(_ context.Context, q beads.ListQuery) ([]domain.Issue, error) {
	f.record("list")
	var out []domain.Issue
	for _, issue := range f.sortedIssues() {
...
func (f *MemClient) Show(_ context.Context, id string, _ beads.Scope) (domain.IssueDetail, error) {
	f.record("show:" + id)
...
func (f *MemClient) Blocked(context.Context, beads.Scope) ([]domain.Issue, error) {
	f.record("blocked")
	var out []domain.Issue
	for _, issue := range f.sortedIssues() {
...
func (f *MemClient) Search(_ context.Context, query string, _ beads.Scope) ([]domain.Issue, error) {
	f.record("search")
...
func (f *MemClient) Stale(_ context.Context, days int, _ beads.Scope) ([]domain.Issue, error) {
	f.record("stale")
...
func (f *MemClient) Stats(context.Context, beads.Scope) (beads.Stats, error) {
...
func (f *MemClient) Create(_ context.Context, in beads.CreateIssueInput) (domain.Issue, error) {
	f.record("create")
...
func (f *MemClient) Update(_ context.Context, id string, in beads.UpdateIssueInput) (domain.Issue, error) {
	f.record("update:" + id)
	issue, ok := f.issues[id]
	if !ok {
...
func (f *MemClient) Claim(_ context.Context, id string, in beads.ClaimInput) (domain.Issue, error) {
	f.record("claim:" + id)
	issue, ok := f.issues[id]
	if !ok {
...
func (f *MemClient) Close(_ context.Context, id string, in beads.CloseInput) (domain.Issue, error) {
	f.record("close:" + id)
	issue, ok := f.issues[id]
	if !ok {
...
func (f *MemClient) Reopen(_ context.Context, id string, _ beads.ReopenInput) (domain.Issue, error) {
	f.record("reopen:" + id)
	issue, ok := f.issues[id]
	if !ok {
...
func (f *MemClient) Assign(_ context.Context, id string, actor string, _ beads.Scope) (domain.Issue, error) {
	f.record("assign:" + id)
	issue, ok := f.issues[id]
	if !ok {
...
func (f *MemClient) AddDependency(_ context.Context, in beads.DependencyInput) error {
	f.record("dep add")
	f.Rel(in.Blocked, in.Blocker, in.Type)
	return nil
}

func (f *MemClient) RemoveDependency(_ context.Context, in beads.DependencyInput) error {
	f.record("dep remove")
...
func (f *MemClient) ListDependencies(_ context.Context, id string, dir beads.Direction, _ beads.Scope) ([]domain.Dependency, error) {
...
func (f *MemClient) Cycles(context.Context, beads.Scope) ([][]string, error) {
...
func (f *MemClient) Prime(context.Context, beads.Scope) (domain.PrimeResult, error) {
	return domain.PrimeResult{Text: "workflow context"}, nil
}

func (f *MemClient) Remember(_ context.Context, in beads.RememberInput) (domain.Memory, error) {
	return domain.Memory{Content: in.Content}, nil
}

func (f *MemClient) Memories(context.Context, string, beads.Scope) ([]domain.Memory, error) {
	return nil, nil
}

func (f *MemClient) Forget(context.Context, string, beads.Scope) error { return nil }

func (f *MemClient) History(context.Context, string, beads.Scope) ([]domain.Event, error) {
	return nil, nil
}

func (f *MemClient) SyncStatus(context.Context, beads.Scope) (domain.SyncStatus, error) {
	return domain.SyncStatus{Available: false, Detail: "no remote configured"}, nil
}

func (f *MemClient) Capabilities(context.Context, beads.Scope) beads.Capabilities {
...
func (f *MemClient) Runner() *beads.Runner { return beads.NewRunner("bd") }

func (f *MemClient) ArgvFor(_ string, args ...string) []string {
	return append([]string{"bd"}, args...)
}

var _ beads.Client = (*MemClient)(nil)
